...
🙋‍♂️ ⁞ إضغط على الزر بالأسفل للتحقق",
	 ...
	 ",
		"10" => " اربح نقاط مجانا 🏆 ",
		"11" => "
- - السَلام عليكُم مرحبا بك في بوت حسابات التيليجرام .
...
• الشحن نقاط عمري : @uutuuu 🗽

- ايدي حسابك : <code>$id</code> 
- رصيدك : $point$", 
		"12" => "
لشحن حسابك قم بالتواصل مع الادارة @c_w_e 
...
",
		"30" => "إعادة طلب كود التفعيل",
		"31" => "❌ تسجيل الخروج من الرقم",
		"32" => "تم تسجيل الخروج بنجاح ✅",
		"33" => "في انتظار الكود...",
...
",
    "30" => "Resend Activation Code",
    "31" => "❌ Logout from Number",
    "32" => "Logged out successfully ✅",
    "33" => "Waiting for code...",
),
"ru" => array(
...
",
    "30" => "Отправить код активации повторно",
    "31" => "❌ Выйти из номера",
    "32" => "Выход выполнен успешно ✅",
    "33" => "Ожидание кода...",
),
...
",
    "30" => "ارسال مجدد کد فعال‌سازی",
    "31" => "❌ خروج از شماره",
    "32" => "خروج با موفقیت انجام شد ✅",
    "33" => "در انتظار کد...",
),
...
if($ex[0] == "/start") {
	if(!isset($points[$id]) && isset($ex[1]) && $id !=  $ex[1]){
...
if($exData[0] == "lang"){
	$langs[$id]=$exData[1];
	saveLangs();
	if(!isset($points[$id])){ // check if it's a new user
        $points[$id]=0;
        if(isset($invite['whoInvitedMe'][$id])){
            $inviter_id = $invite['whoInvitedMe'][$id];
            if(isset($points[$inviter_id])) { // check if inviter exists
                $points[$inviter_id] += $invitePoint;
                $invite['invited'][$inviter_id][] = $id; // Store multiple invited users
                saveInvite();

                // Check if inviter has a language set, otherwise default to 'ar' for notification
                $inviter_lang = $langs[$inviter_id] ?? 'ar';
                $ty = $trans[$inviter_lang];
                $tx = str_replace("__invitePoint__", $invitePoint, $ty[4]);
                send($tx, null, $inviter_id);
            }
        }
        savePoint();
    }
...
if(!check_member($id,$ch5)) {
	$link = "https://t.me/lllll5lbot?start=0";
...
$communication = array(
	 ...
	 "ru" => "Свяжитесь с разработчиком",
	 "fa" => "با توسعه دهنده تماس بگیرید",
	 "cht" => "聯繫開發者",
	 "chb" => "联系开发者"
);
...
if ($text == "/start" || $ex[0] == "/start") {
	...
	$btn = mkBtn ([
        ...
        array(
            "{$communication[$langs[$id]]}" => "communication"
        ),
        array(
...
} else if ( $exData[0] == "getCode" || $exData[0] == "resend_code") {
	$hashCode = $exData[1];
	$k = $exData[2];
	$get = $api->getCode($hashCode);
	if (is_array($get) && !empty($get['code'])) {
		$code = $get['code'];
		$pass = $get['password'];
		$stats['all']['buy'] += 1;
		$stats[$id]['buy'] += 1;
		...
		$btn = mkBtn (
			array(
				array (
					"{$txt[30]}" => "resend_code#{$hashCode}#$k#$num",
					"{$txt[31]}" => "logout_number#{$hashCode}"
				)
			)
		);
...
";
		if ( $exData[0] == "getCode") send($tx,null,$ch2);
		$idd = substr("$id",0,-4) . "••••";
		$num = substr("$num",0,-4) . "••••";
                $p=$contries[$exData[2]]??0;
		$tx = "
...
if ( $exData[0] == "getCode") send($tx,[[["text" => "🤖 شراء رقم من البوت 🤖","url" => "https://t.me/lllll5lbot"]]],$ch3);
	} else {
		bot('answercallbackquery',[
			'callback_query_id'=>$update->callback_query->id,
			'show_alert'=>true,
...
} else if ($exData[0] == "logout_number") {
	$hashCode = $exData[1];
	$api->getCode($hashCode, 'yes');
	bot('answercallbackquery',[
		'callback_query_id'=>$update->callback_query->id,
		'show_alert'=>true,
		'text' => $txt[32],
	]);
	edit("✅ تم تسجيل الخروج من الرقم بنجاح.");
}
if ($data == "communication") {
    if (!file_exists("communication.txt")) {
        bot("EditMessageText", [
            "chat_id" => $chat_id,
    ...
    bot("EditMessageText", [
        "chat_id" => $chat_id,
        'message_id' => $message_id,
        "text" => "[ᶠʳᵒᵐ ʲᵘˢᵗ ᴬᵇᵈᵒ](tg://user?id=6969088145)\nحسنا ارسل ما تريده للمالك",
        'parse_mode' => "markdown",
        'reply_markup' => json_encode([
            'inline_keyboard' => [
                [['text' => "• الغاء •", 'callback_data' => "elgaa"]],
            ]
        ])
    ]);
    file_put_contents("communication_" . $id . ".txt","on");
    exit;
} else if ($data == "elgaa") {
    bot("EditMessageText", [
        "chat_id" => $chat_id,
    ...
    unlink("communication_" . $id . ".txt");
    exit;
} else if ($text and trim(file_get_contents("communication_" . $id . ".txt")) === "on") {
    bot("sendmessage", [
        "chat_id" => $chat_id,
    ...
    bot("sendmessage", [
        "chat_id" => $admin,
        "text" => "رساله من العضو $name / $id \n" . $text,
        'parse_mode' => "markdown",
        'reply_markup' => json_encode([
            'inline_keyboard' => [
                [['text' => "• الرد على الرساله •", 'callback_data' => $id . "|Reply_message"]],
            ]
        ])
    ]);
    unlink("communication_" . $id . ".txt");
